    ...
    public function register(Request $request, UserPasswordEncoderInterface $passwordEncoder, UserRepository $userRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->emailVerifier->sendEmailConfirmation('app_verify_email', $user,
                (new TemplatedEmail())
                    ->from(new Address('dyonisos40games@gmail.com', 'Confirmation Mail'))
                    ->to($user->getEmail())
                    ->subject('Confirmez votre email s\'il vous plaît')
                    ->htmlTemplate('registration/confirmation_email.html.twig')
            );
            // do anything else you need here, like send an email
            $this->addFlash('success', 'Connectez vous et un email vient d\'être envoyer, merci de confirmer votre inscription.');
